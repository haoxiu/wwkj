...
@interface ZhaoPinViewController ()<UITableViewDataSource,UITableViewDelegate,UIPickerViewDataSource,UIPickerViewDelegate>
{
    NSArray *_types;
...
//    _scrollerView.userInteractionEnabled = YES;
    ...
    _development.keyboardType = UIKeyboardTypeNumberPad;
    _phone.keyboardType = UIKeyboardTypeNumberPad;
    
    _picker = [[UIPickerView alloc]initWithFrame:CGRectMake(0,[UIScreen mainScreen].bounds.size.height - 64, self.view.width, 100 +64)];
...
- (void)_loadSubViews {
    _types = @[@"兼职",@"全职",@"其他"];
    _exlArr =@[@"1年以下",@"1年",@"2年",@"3年",@"4年",@"5年",@"5年以上"];
    _education = @[@"初高中以下",@"大专",@"本科",@"硕士",@"博士"];
    
    _smallTableView =[[UITableView alloc]initWithFrame:CGRectMake(0,self.view.height - 64, self.view.width, 1/3.0*self.view.height) style:UITableViewStyleGrouped];
...
- (IBAction)publishAction:(UIButton *)sender {
    if (_companyName.text.length == 0 || _phone.text.length ==0) {
        ...
        NSDictionary *params = @{
                                 ...
                                 @"workback":_expL.text,
                                 @"educational":_educationL.text,
                                 @"salary":_development.text,
                                 @"place":_workplaceL.text,
                                 @"jobtype":_position.text,
                                 @"phone":_phone.text,
        ...
        [CYNetworkTool post:URL_PublishZp params:params success:^(id json) {
...
- (void)textViewDidBeginEditing:(UITextView *)textView {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        self.view .transform = CGAffineTransformMakeTranslation(0, -fabs(textView.y+130-(self.view.height - 216)));
    }];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        if (cell.textLabel.text ==_exlArr[indexPath.row]){
            _expL.text =cell.textLabel.text;
        }else{
            _educationL.text =cell.textLabel.text;
        }
...
- (IBAction)cilckButtonChange:(UIButton *)sender {
    ...
    [shadeView setFrame:CGRectMake(0, - 64, CYWindowWidth, CYWindowHeight)];
...
- (IBAction)place:(id)sender {
    ...
    [locationView setFrame:CGRectMake(0, - 64, CYWindowWidth, CYWindowHeight)];
    ...
    [UIView animateWithDuration:.5 animations:^{
        _picker.transform =CGAffineTransformMakeTranslation(0, -_picker.height);
        confirm.transform =CGAffineTransformMakeTranslation(0, -_picker.height +30);
    }];
}

- (void)confirmLocation:(UIButton *)button {
    _workplaceL.text =[NSString stringWithFormat:@"%@%@%@",province,city,area];
