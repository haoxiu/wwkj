...
//            NSString *s = [[NSUserDefaults standardUserDefaults]objectForKey:@"hdimg"];
            ...
            NSString *substring1 = [hdimg substringFromIndex:22];
            NSString *substring2 = [substring1 substringToIndex:16];
            NSString *substring3 = [substring1 substringFromIndex:17];
            NSString *filePath = [[NSBundle mainBundle] pathForResource:substring2 ofType:substring3];
            NSData *image = [NSData dataWithContentsOfFile:filePath];
            //            NSData *_decodedImageData   = [[NSData alloc] initWithBase64Encoding:hdimg];
            _headImg = [UIImage imageWithData:image];
            ...
            _hv.nickImg.layer.cornerRadius = 5;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0){
        ...
        BOOL login = [userDefaults boolForKey:@"islogin"];
        if (login) {
...
- (void)theLogin:(UIButton *)send
{
    ...
    LoginViewController *loginV = [profileSB instantiateViewControllerWithIdentifier:@"LoginViewController"];
    [self.navigationController pushViewController:loginV andHideTabbar:YES animated:YES];
}
#pragma mark 单元格高度
